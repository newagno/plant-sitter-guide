...
for p in plants:
    cards_html += f"""
      <article class="plant-card group relative flex flex-col bg-[#FFFCF0]/88 border border-white/85 shadow-[0_30px_70px_rgba(42,73,42,0.13),inset_0_1px_0_rgba(255,255,255,0.9)] overflow-hidden hover:-translate-y-2 hover:rotate-[0.35deg] hover:shadow-[0_38px_90px_rgba(35,72,42,0.2),inset_0_1px_0_rgba(255,255,255,1)] transition-all duration-700">
        <div class="relative m-3 mb-0 h-64 shrink-0 overflow-hidden rounded-[2.5rem_1.7rem_3rem_1.9rem] shadow-[inset_0_-40px_80px_rgba(245,238,213,0.22)]">
          <button type="button" aria-label="Збільшити фото {p['name']}" class="zoom-trigger block w-full h-full p-0 m-0 border-none bg-transparent cursor-zoom-in group-hover:scale-[1.035] transition-transform duration-[1200ms] focus:outline-none focus-visible:ring-4 focus-visible:ring-emerald-500/50 z-20 relative">
            <img src="./assets/images/{p['img']}" alt="" width="600" height="400" loading="lazy" decoding="async" class="w-full h-full object-cover saturate-[1.06] contrast-[1.02]">
          </button>
        ...
        <div class="flex flex-col flex-1 px-8 pb-8 pt-5 relative z-30">
          <div class="mb-4">
            <h3 class="text-[#102B18] font-serif font-bold text-3xl tracking-tight mb-1">{p['name']}</h3>
            <p class="text-xs text-[#72876B] italic font-serif tracking-wide">{p['latin']}</p>
          </div>
          <div class="text-sm text-[#334937] font-sans leading-relaxed flex-1 mb-6">
            {p['text']}
          </div>
          <div class="mt-auto pt-4 border-t border-[#DCE8D2]/80">
            <span class="text-[10px] text-[#6D8267] font-bold flex items-center gap-2 uppercase tracking-widest">
...
<head>
  ...
  <link href="./output.css?v=7" rel="stylesheet">
  ...
  <style>
    ...
    body {{
      background:
        radial-gradient(circle at 14% 8%, rgba(255, 248, 207, 0.98), transparent 26rem),
        radial-gradient(circle at 88% 4%, rgba(198, 232, 183, 0.62), transparent 34rem),
        linear-gradient(135deg, #fff8df 0%, #eef5d8 44%, #dfeccd 100%);
    }}
    body::before {{
      content: "";
      position: fixed;
      ...
      mask-image: radial-gradient(circle at 48% 20%, black, transparent 72%);
      opacity: .38;
    }}
    .sun-room {{
      position: fixed;
    ...
    .window-glow {{
      ...
      height: min(74vw, 58rem);
      border-radius: 999px;
      background: radial-gradient(circle, rgba(255,251,213,.96) 0%, rgba(255,244,178,.55) 28%, rgba(255,255,255,0) 68%);
      filter: blur(2px);
    }}
    .rainbow-beam {{
      position: absolute;
      ...
      border-radius: 999px;
      background: linear-gradient(90deg, rgba(255,125,155,.18), rgba(255,208,105,.24), rgba(160,221,141,.22), rgba(106,209,224,.18), rgba(170,136,255,.16));
      filter: blur(22px);
    ...
    .crystal {{
      ...
      background: linear-gradient(145deg, rgba(255,255,255,.82), rgba(183,238,214,.38), rgba(255,205,226,.34));
      box-shadow: inset 0 0 18px rgba(255,255,255,.9), 0 18px 38px rgba(69,108,75,.14);
    ...
    .crystal::before {{
      ...
      width: 1px;
      height: 3.8rem;
      background: rgba(104,126,96,.24);
    }}
    .crystal.one {{ right: 7.5rem; }}
    .crystal.two {{ right: 12rem; top: 3rem; transform: rotate(-8deg) scale(.76); }}
    .crystal.three {{ right: 3.8rem; top: 4.4rem; transform: rotate(20deg) scale(.62); }}
    .leaf-haze {{
      position: absolute;
    ...
    .plant-card:nth-child(3n+2) {{ border-radius: 2.6rem 4.1rem 2.9rem 4.4rem; }}
    .plant-card:nth-child(4n) {{ border-radius: 4.6rem 2.7rem 3.4rem 2.9rem; }}
    .plant-card::before {{
      content: "";
      position: absolute;
      inset: -1px;
      border-radius: inherit;
      background: radial-gradient(circle at 22% 8%, rgba(255,255,255,.82), transparent 28%), linear-gradient(145deg, rgba(255,255,255,.42), transparent 44%);
      pointer-events: none;
      z-index: 1;
    }}
    .plant-card > * {{ position: relative; z-index: 2; }}
    .care-note {{
      border-radius: 2.2rem 3.1rem 2.4rem 3.3rem;
...
<body class="bg-[#F7F0DE] text-[#334937] font-sans min-h-screen antialiased selection:bg-emerald-200/70 relative overflow-x-hidden">
  ...
  <div class="sun-room" aria-hidden="true">
    ...
    <div class="rainbow-beam one"></div>
    <div class="rainbow-beam two"></div>
    <div class="rainbow-beam three"></div>
    <div class="crystal one"></div>
    <div class="crystal two"></div>
    <div class="crystal three"></div>
    <div class="leaf-haze left"></div>
  ...
  <main class="container mx-auto px-6 py-14 max-w-screen-2xl relative z-10">
    
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-8 gap-y-14">
{cards_html}
    </div>
    
    <!-- Terrarium Banner -->
    <div class="mt-28 relative bg-white/62 backdrop-blur-3xl border border-white/90 rounded-[4rem_2.6rem_4.6rem_2.9rem] p-10 lg:p-16 shadow-[0_30px_90px_rgba(49,83,51,0.13),inset_0_1px_0_rgba(255,255,255,0.9)] overflow-hidden">
      ...
      <div class="relative z-10">
        ...
        <div class="grid md:grid-cols-2 gap-10 lg:gap-16 text-[#3B4D3E] font-sans leading-relaxed">
          ...
          <div class="bg-rose-50/78 border border-rose-100/90 rounded-[2.8rem_2rem_3.2rem_2.2rem] p-8 backdrop-blur-xl shadow-[inset_0_1px_0_rgba(255,255,255,0.8)]">
  ...
  <div id="zoomModal" role="dialog" aria-modal="true" aria-label="Збільшене зображення" tabindex="-1" class="fixed inset-0 z-50 flex items-center justify-center bg-[#F3EACF]/88 backdrop-blur-xl opacity-0 pointer-events-none transition-all duration-300">
  ...
  <script>
    ...
    function updateCountdown() {{
      ...
      if (days > 0) {{
        el.innerHTML = `${{days}}д ${{hours}}г`;
      }} else {{
        el.innerHTML = `${{hours}}г ${{minutes}}хв`;
      }}
    }}
